...
def get_point_list( str_file_name, list_chiban ):
    ...
    print("フェイズ６")
    # print(g_zahyo_list)
    return (g_point_list, g_zahyo_list)
...
if __name__ == '__main__':
    ...
    book = openpyxl.load_workbook(excel_file_name, \
            data_only=True)
    sheet = book.active
    str_file_name = sheet.cell(row=2, column=2).value
    list_chiban = []
    num_row = 4
    while (True):
        str_ooaza = sheet.cell(row=num_row, column=2).value
        if str_ooaza == None:
            break
        str_choume = sheet.cell(row=num_row, column=3).value
        if str_choume == None:
            str_choume = ""
        str_shouji = sheet.cell(row=num_row, column=4).value
        if str_shouji == None:
            str_shouji = ""
        str_chiban = sheet.cell(row=num_row, column=5).value
        if str_chiban == None:
            str_chiban = ""
        list_chiban.append( \
                    (str_ooaza, str_choume, \
                    str_shouji, str_chiban))
        num_row += 1
    
    list_point = get_point_list(str_file_name, list_chiban)
    if (sheet.cell(row=2, column=3).value == "display"):
        display()
    
    
    num_row += 5
    for point_pair in zip(g_point_list, g_zahyo_list):
        sheet.cell(row=num_row, column=1).value = point_pair[0]
        sheet.cell(row=num_row, column=3).value = point_pair[1][0]
        sheet.cell(row=num_row, column=4).value = point_pair[1][1]
        num_row += 1
